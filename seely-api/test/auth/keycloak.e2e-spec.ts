...
describe('Keycloak Authentication (e2e)', () => {
  ...
  describe('Keycloak Authentication', () => {
    ...
    it('/keycloak/login should redirect to Keycloak or return error', async () => {
      ...
      if (response.status === 302) {
        ...
        const cookies = Array.isArray(setCookieHeader)
          ...
          : [setCookieHeader];
        expect(
          cookies.some((cookie) => cookie.includes('state=')),
        ).toBeTruthy();
        expect(
          cookies.some((cookie) => cookie.includes('codeVerifier=')),
        ).toBeTruthy();
    ...
    it('/keycloak/callback should handle authorization code or return error', async () => {
      ...
      const loginResponse = await request(getApp().getHttpServer()).get(
        '/keycloak/login',
      );
      ...
      if (loginResponse.status === 302) {
        ...
        if (stateCookie) {
          ...
          const response = await request(getApp().getHttpServer())
            .get(
              `/keycloak/callback?code=${mockCode}&state=${state}&session_state=mock&iss=mock`,
            )
        ...
        const response = await request(getApp().getHttpServer()).get(
          '/keycloak/callback?code=test&state=test',
        );
    ...
    it('/keycloak/callback should reject invalid state or return service error', async () => {
      ...
      const response = await request(getApp().getHttpServer()).get(
        `/keycloak/callback?code=${mockCode}&state=${invalidState}`,
      );
  ...
  describe('Keycloak Logout Flow', () => {
    ...
    it('/keycloak/logout should redirect to logout success', async () => {
      ...
      if (setCookieHeader) {
        ...
        const hasRefreshTokenClear = cookies.some((c) =>
          c.includes('refreshToken=;'),
        );
        const hasAccessTokenClear = cookies.some((c) =>
          c.includes('accessToken=;'),
        );

        // If any cookies are being cleared, that's good
        const hasCookieClearing =
          hasIdTokenClear || hasRefreshTokenClear || hasAccessTokenClear;
        ...
        console.log(
          'No cookie clearing headers - cookies cleared by Keycloak service',
        );
        expect(true).toBe(true);
      }
    });
    ...
    it('/keycloak/logout without idToken should redirect to home', async () => {
      ...
      const isValidRedirect =
        location.includes('/logout-success') ||
